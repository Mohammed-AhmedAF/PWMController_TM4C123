...
void PWM_vidInit(PWMConfig_t * ptPWMConfig)
{
		...
		/*Action*/
		PWM_vidSetOutput(ptPWMConfig->u8Module,ptPWMConfig->u8Generator,ptPWMConfig->u8Block,ptPWMConfig->u16Output);
	...
	if (u8Module == PWM_MODULE_0)
	{
		switch(u8Generator)
		{
			case PWM_GENERATOR_0:
					...
					break;
					case PWM_GENERATOR_1:
						if (u8Channel == PWM_CHANNEL_2)
						{
						PWM0_CMPA1 = u16CompVal;
						}
						else if (u8Channel == PWM_CHANNEL_3)
						{
						PWM0_CMPB1 = u16CompVal;
						}
					break;
				case PWM_GENERATOR_2:
						if (u8Channel == PWM_CHANNEL_4)
						{
						PWM0_CMPA2 = u16CompVal;
						}
						else if (u8Channel == PWM_CHANNEL_5)
						{
						PWM0_CMPB2 = u16CompVal;
						}
					break;
				case PWM_GENERATOR_3:
					if (u8Channel == PWM_CHANNEL_6)
						{
						PWM0_CMPA3 = u16CompVal;
						}
						else if (u8Channel == PWM_CHANNEL_7)
						{
						PWM0_CMPB3 = u16CompVal;
						}
					break;
...
void PWM_vidSetOutput(u8 u8Module, u8 u8Generator, u8 u8Block, u16 u16Output)
{
	if (u8Module == PWM_MODULE_0)
	{
		switch (u8Generator)
